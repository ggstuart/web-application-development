...
In this exercise we have created a user interface skeleton for a list of *todo-lists* which will be the landing page for our application.
We gave the page some structure by adding semantic `<header>`, `<main>` and `<footer>` elements.
We added a simple list as the main content with each list item containing the title of the list as a second level heading.

> Try navigating the page with a screen reader system, it should work relatively well.